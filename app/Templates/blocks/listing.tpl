		...
		<table id='BlocksTable' class='list tablesorter'>
			...
			<tbody>
				...
				<tr class='radek1'>
					<td>
						<a href='?id=<?php echo $row['id']; ?>&cms=edit&page=block' title='Upravit'>
							<img class='edit' src='<?php echo IMG_DIR; ?>icons/edit.gif' />
						</a>
					</td>
					<td>
						<a href="javascript:confirmation('?id=<?php echo $row['id']; ?>&amp;cms=delete', 'blok: <?php echo $row['name']; ?> <?php echo $row['from']; ?> -> Opravdu SMAZAT tento blok? Jste si jisti?')" title='Odstranit'>
