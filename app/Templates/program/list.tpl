		...
		<div class='link'>
			<a class='link' href='?program&cms=new&page=program'>
		...
		<table id='ProgramsTable' class='list tablesorter'>
			...
			<tbody>
				...
				<tr class='radek1'>
					<td>
						<a href='?program&id=<?php echo $row['id']; ?>&cms=edit&page=program' title='Upravit'>
							<img class='edit' src='<?php echo IMG_DIR; ?>icons/edit.gif' />
						</a>
					</td>
					<td>
						<a href='?program&cms=export-visitors&id=<?php echo $row['id']; ?>' title='Účastníci programu'>
							<img class='edit' src='<?php echo IMG_DIR; ?>icons/pdf.png' />
						</a>
					</td>
					<td>
						<a href="javascript:confirmation('?program&id=<?php echo $row['id']; ?>&amp;cms=delete', 'program: <?php echo $row['name']; ?> -> Opravdu SMAZAT tento program? Jste si jisti?')" title='Odstranit'>
